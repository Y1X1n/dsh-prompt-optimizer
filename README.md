...
- 发送栏工具行右侧新增「优化」按钮(输入为空时禁用,优化中带呼吸动画);等待期间面板实时显示当前阶段(等待模型响应 / 分析诊断 / 输出优化稿)与已用秒数。
...
- **轻量记忆链**:在我们产出的优化稿上继续修改后再点优化,自动携带上一轮结果作为延续参考(沿用已确认决策,只围绕变化点调整);同文重试不带;发送或关闭面板后自然归零。
...
- 自动化测试(`npm test`,共 53 例):
  ...
  - `test/controller.test.mjs`:14 项客户端纯逻辑用例(SSE 帧解析、合帧节流、连接中断、跳过会话查询、撤回流转、retry、close 中止、历史提取过滤与失败降级、斜杠前缀拆分、记忆链传递)。
