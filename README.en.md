...
- **Session query face**: `connection.api.sessions.*` / `connection.api.llm.models` were never declared in any published release (0.1.0-rc.7 → 0.1.5) and are absent from the browser runtime; the plugin calls them through an optional structural face and degrades per call (model routing falls back to Host resolution, context is treated as absent), with a matching Console warning.
